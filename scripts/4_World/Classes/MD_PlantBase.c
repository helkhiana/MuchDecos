class MD_PlantBase extends ItemBase
{
	// Plant states
	...
	private MD_GardenBase m_MD_GardenBase = NULL;
	private ref MD_Slot m_Slot = NULL;
	...
	void Init( MD_GardenBase garden_base, float fertility, float harvesting_efficiency, float water )
	{
		m_MD_GardenBase = garden_base;
	...
	override bool OnStoreLoad( ParamsReadContext ctx, int version )
	{
		...
		MD_GardenBase garden = MD_GardenBase.Cast( GetHierarchyParent() );
		Print(garden);
		...
		MD_Slot slot = garden.GetSlotByIndex(slot_index);
		Print(slot);
